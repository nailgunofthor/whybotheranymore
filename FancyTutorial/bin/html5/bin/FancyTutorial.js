...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("img/hungryTaraIcon.png",loader);
	ApplicationMain.total++;
	var loader1 = new flash.display.Loader();
	ApplicationMain.loaders.set("img/pizzaIcon.png",loader1);
	ApplicationMain.total++;
	var loader2 = new flash.display.Loader();
	ApplicationMain.loaders.set("img/shipIcon.png",loader2);
	ApplicationMain.total++;
	var loader3 = new flash.display.Loader();
	ApplicationMain.loaders.set("img/tinymap.png",loader3);
	ApplicationMain.total++;
...
var Main = function() {
	...
	flash.Lib.get_current().get_stage().addEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(this,this.processKeyPressed));
	flash.Lib.get_current().get_stage().addEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(this,this.processKeyReleased));
	flash.Lib.get_current().get_stage().addEventListener(flash.events.Event.ENTER_FRAME,$bind(this,this.doMoreStuffIGuess));
	this.slices = new List();
	this.tara = new HungryTara(400,40);
	this.addChild(this.tara);
...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	...
	,doMoreStuffIGuess: function(e) {
		if(this.leftDown) this.ship.goLeft();
		if(this.rightDown) this.ship.goRight();
		this.ship.act();
		var $it0 = this.slices.iterator();
		while( $it0.hasNext() ) {
			var slice = $it0.next();
			slice.act();
		}
	}
	,processKeyReleased: function(e) {
		console.log(e.keyCode);
		if(e.keyCode == 37) this.leftDown = false;
		if(e.keyCode == 39) this.rightDown = false;
	}
	,processKeyPressed: function(e) {
		console.log(e.keyCode);
		if(e.keyCode == 37) this.leftDown = true;
		if(e.keyCode == 39) this.rightDown = true;
		if(e.keyCode == 38) this.ship.shoot();
	}
	,init: function() {
...
var DefaultAssetLibrary = function() {
	...
	DefaultAssetLibrary.path.set("img/hungryTaraIcon.png","img/hungryTaraIcon.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("img/hungryTaraIcon.png",value);
	DefaultAssetLibrary.path.set("img/pizzaIcon.png","img/pizzaIcon.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("img/pizzaIcon.png",value);
	DefaultAssetLibrary.path.set("img/shipIcon.png","img/shipIcon.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("img/shipIcon.png",value);
	DefaultAssetLibrary.path.set("img/tinymap.png","img/tinymap.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("img/tinymap.png",value);
};
...
var HungryTara = function(x,y) {
	this.FULL_TUMMY = 10;
	flash.display.Sprite.call(this);
	this.hunger = 0;
	this.isSatisfied = false;
	var hungryTaraIcon = new flash.display.Bitmap(openfl.Assets.getBitmapData("img/hungryTaraIcon.png"));
	var taraSprite = new flash.display.Sprite();
	taraSprite.addChild(hungryTaraIcon);
	taraSprite.set_width(taraSprite.set_height(100));
	taraSprite.set_x(-taraSprite.get_width() / 2);
	taraSprite.set_y(-taraSprite.get_height() / 2);
	this.addChild(taraSprite);
	this.refx = x;
	this.hungerBar = new flash.display.Sprite();
	this.hungerBar.get_graphics().beginFill(65280,2);
	this.hungerBar.get_graphics().drawRect(0,0,hungryTaraIcon.get_width(),5);
	taraSprite.addChild(this.hungerBar);
	this.hungerBar.set_y(-8);
	this.set_width(this.set_height(100));
	this.set_x(x);
	this.set_y(y);
};
$hxClasses["HungryTara"] = HungryTara;
HungryTara.__name__ = ["HungryTara"];
HungryTara.__super__ = flash.display.Sprite;
HungryTara.prototype = $extend(flash.display.Sprite.prototype,{
	act: function() {
		var randomNum = Math.random() * 800;
		motion.Actuate.tween(this,2,{ x : randomNum}).onComplete($bind(this,this.act));
	}
	,__class__: HungryTara
});
...
var Pizza = function(x,y) {
	flash.display.Sprite.call(this);
	var pizzaIcon = new flash.display.Bitmap(openfl.Assets.getBitmapData("img/pizzaIcon.png"));
	var pizzaSprite = new flash.display.Sprite();
	pizzaSprite.addChild(pizzaIcon);
	pizzaSprite.set_x(-pizzaIcon.get_width() / 2);
	pizzaSprite.set_y(-pizzaIcon.get_height() / 2);
	this.addChild(pizzaSprite);
	this.set_width(25);
	this.set_height(25);
	this.set_x(x);
	this.set_y(y);
	this.countdownToConsumption = -1;
};
$hxClasses["Pizza"] = Pizza;
Pizza.__name__ = ["Pizza"];
Pizza.__super__ = flash.display.Sprite;
Pizza.prototype = $extend(flash.display.Sprite.prototype,{
	act: function() {
		if(this.countdownToConsumption < 0) {
			var distanceFromTara = Math.sqrt((this.get_x() - Main.game.tara.get_x()) * (this.get_x() - Main.game.tara.get_x()) + (this.get_y() - Main.game.tara.get_y()) * (this.get_y() - Main.game.tara.get_y()));
	...
	,feed: function() {
		this.countdownToConsumption = 60;
...
var Ship = function(x,y) {
	...
	flash.display.Sprite.call(this);
	this.isAlive = true;
	var playerSprite = new flash.display.Sprite();
	var shipIcon = new flash.display.Bitmap(openfl.Assets.getBitmapData("img/shipIcon.png"));
	playerSprite.addChild(shipIcon);
	playerSprite.set_x(-shipIcon.get_width() / 2);
	playerSprite.set_y(-shipIcon.get_height() / 2);
...
$hxClasses["Ship"] = Ship;
Ship.__name__ = ["Ship"];
Ship.__super__ = flash.display.Sprite;
Ship.prototype = $extend(flash.display.Sprite.prototype,{
	act: function() {
		if(this.get_x() < this.get_width() / 2 && this.velocity < 0) {
			this.velocity = 0;
			this.set_x(this.get_width() / 2);
		}
		if(this.get_x() > 800 - this.get_width() / 2 && this.velocity > 0) {
			this.velocity = 0;
			this.set_x(800 - this.get_width() / 2);
		}
		this.velocity *= 0.9;
		var _g = this;
		_g.set_x(_g.get_x() + this.velocity);
	}
	,shoot: function() {
		if(this.isAlive) {
			var slice = new Pizza(this.get_x() | 0,this.get_y() - this.get_height() / 2 | 0);
